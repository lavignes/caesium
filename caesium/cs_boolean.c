...
static CsValue __true(CsMutator* mut, CsValue self) {
  return CS_TRUE;
}
...
CsValue cs_initclass_true(CsMutator* mut) {
  ...
  cs_hash_insert(dict, "__bool", 6, cs_mutator_new_builtin1(mut, __true));
...
static CsValue __false(CsMutator* mut, CsValue self) {
  return CS_FALSE;
}
...
CsValue cs_initclass_false(CsMutator* mut) {
  ...
  cs_hash_insert(dict, "__bool", 6, cs_mutator_new_builtin1(mut, __false));
